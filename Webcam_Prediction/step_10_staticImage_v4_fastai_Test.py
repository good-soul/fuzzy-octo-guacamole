...
# image = open_image('intermediate.png')
# image = cv2.imread('intermediate.png', 0)
...
crop_img = cv2.resize(image, (50, 50))
# crop_img = crop_img.reshape(1, 1, 50, 50).astype(np.float32)

tmpFile = str(int(time.time()))+'.png'

# cv2.imwrite(tmpFile, crop_img)
cv2.imwrite("intermediate2.png", crop_img)

# img = open_image(tmpFile)
img = open_image("intermediate2.png")
